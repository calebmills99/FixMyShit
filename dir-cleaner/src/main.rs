...
fn main() {
    ...
    print_tree(&folder, 0, &mut HashMap::new());
    println!();
...
fn print_tree(path: &Path, depth: usize, last_at_depth: &mut HashMap<usize, bool>) {
    ...
    if path.is_dir() {
        ...
        for (i, entry) in entries.iter().enumerate() {
            ...
            print_tree(&entry.path(), depth + 1, last_at_depth);
        }
    }
}
